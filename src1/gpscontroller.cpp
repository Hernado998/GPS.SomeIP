...
void GpsController::onDataReceived(double p_lat, double p_lon,int p_hour,int p_minute,int p_second,int p_day,int p_month,int p_year,int p_speed)
{
	std::cout << "Data received: " << p_lat << ", " << p_lon <<" ||| "<<p_hour<<":"<<p_minute<<":"<<p_second<< " ||| "<<p_day<<"/"<<p_month<<"/"<<p_year<< " ||| "<<p_speed<<std::endl;
	m_logFile << p_lat << ", " << p_lon <<" ||| "<<p_hour<<":"<<p_minute<<":"<<p_second<< " ||| "<<p_day<<"/"<<p_month<<"/"<<p_year<< " ||| "<<p_speed<<std::endl;
}
